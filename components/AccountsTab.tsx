...
export default function AccountsTab({ accounts }: AccountsTabProps) {
  ...
  async function fetchAccountHistory(accountId: string) {
    ...
    const combined = [
      ...(exp || []).map(e => ({ ...e, type: 'gasto', date: new Date(e.created_at) })),
      ...(trans || []).map(t => ({ ...t, type: 'transfer', date: new Date(t.created_at) }))
              ...
              {loadingHistory ? (
                ...
                <div key={i} className="flex justify-between items-center border-b border-zinc-800/50 pb-3">
                  ...
                  <p className={`text-xs font-mono font-black ${
                    item.type === 'gasto' || item.from_account_id === selectedAcc.id ? 'text-red-400' : 'text-green-400'
                  }`}>
                    {item.type === 'gasto' || item.from_account_id === selectedAcc.id ? '-' : '+'}${item.amount}
                  </p>
                </div>
              ))}
