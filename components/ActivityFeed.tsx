    ...
    <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-[2.5rem] shadow-2xl h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto pr-2 no-scrollbar space-y-6 relative before:absolute before:left-3 before:top-2 before:bottom-2 before:w-[1px] before:bg-zinc-800/50">
        ...
        {combinedActivity.map((item) => {
          ...
          // IMPORTANTE: Ahora detectamos income si viene de la tabla incomes O si el type en expenses es 'income'
  ...
  return (
    ...
    >
        ...
        <div className={`absolute left-0 top-1 w-6 h-6 bg-black border rounded-full flex items-center justify-center z-10 ${
          ...
          isTransfer ? 'border-blue-500/50' : 
          isIncome ? 'border-emerald-500/50' : 'border-red-500/50'
        }`}>
        ...
        <div className="flex flex-col">
          {/* Título: Categoría o Tipo de movimiento */}
          <p className="text-xs font-bold text-zinc-200">
            {isTime ? `Sesión de ${cat?.name}` : isTransfer ? "Transferencia" : isIncome ? `Ingreso: ${cat?.name || ''}` : `Gasto: ${cat?.name || ''}`}
          </p>

          {/* Edición de Descripción / Nota */}
          <div className="flex items-center gap-2 mt-0.5 group/actions">
            {isEditing ? (
              ...
              <>
                <p className="text-[10px] text-zinc-500 italic leading-tight truncate max-w-[160px]">
                ...
                {!isTime && (
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => { setEditingId(item.id); setEditValue(item.description || ""); }}>
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 text-zinc-600 hover:text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                    </button>
                    <button onClick={() => handleDelete(item.id, item.feedType)}>
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 text-zinc-600 hover:text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                    </button>
                  </div>
                )}
              </>
        ...
        <div className="flex flex-wrap gap-2 items-center mt-1.5">
          ...
          <span className={`text-[9px] px-2 py-0.5 rounded-full font-black shrink-0 ${
            ...
            {isTime ? "TERMINADO" : isIncome ? `+$${item.amount.toLocaleString()}` : `-$${item.amount.toLocaleString()}`}
          </span>

          {/* NOMBRE DE LA CUENTA O RUTA DE TRANSFERENCIA */}
          {!isTime && (
            <span className="text-[10px] font-black text-purple-500 uppercase tracking-tighter">
              {isTransfer ? (
                // Si es transferencia, muestra Origen ➔ Destino
                <span className="flex items-center gap-1">
                  {item.from?.name} <span className="text-zinc-600 text-[8px]">➔</span> {item.to?.name}
                </span>
              ) : (
                // Si es gasto o ingreso, muestra la cuenta normal
                item.accounts?.name || item.account_name
              )}
            </span>
          )}
